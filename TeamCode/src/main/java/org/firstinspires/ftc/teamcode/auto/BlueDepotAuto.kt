...
@Deprecated("")
@Disabled
@Autonomous(name = "蓝方 - 营地", group = "Auto")
class BlueDepotAuto: BaseOpMode(true, true, false, true,
        true, true, true) {
...
//        chassis.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE)//开启刹车便于控制
        val goldMineralPosition = tensorFlow.getGoldPositionNew()
        waitForStart()
        welcome()
        val toBottom = Thread {
...
//        tensorFlow.activate()
        ...
        when (gold) {
            ...
            3 -> {
                ...
                Thread {
                    chassis.setTransverseByEncoder(-300, 1800)
                }.start()
                collect.outForAuto()
                Thread {
                    arm.middleForAuto()
                }.start()
                chassis.setStraightByEncoder(3500, 5000)
            }//右边
        }
    }
